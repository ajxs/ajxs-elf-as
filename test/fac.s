...
main:
  ...
  addiu   $sp, $sp, -12     # move stackpointer up 3 words
  ...
  addiu   $sp, $sp, 12      # move stack pointer back down where we started
...
factorial:
  ...
  addiu   $t0, $t0, -1      # subtract 1 from $t0 if not equal to 0

  # recursive case - move to this call's stack segment
  addiu   $sp, $sp, -12     # move stack pointer up 3 words
...
# return x * factorial(x-1); (not the return statement, but the multiplication)
  ...
  addiu   $sp, $sp, 12      # move stackpointer back down for the parent call
